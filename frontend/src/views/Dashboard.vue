...
export default {
  ...
  methods: {
    ...
    async doCreate() {
      ...
      this.actionLoading = true
      try {
        await axios.post(`/servers/${this.currentServer}/create/${this.createNum}`, {
    ...
    async doAction(action, id) {
      ...
      try {
        if (action === 'delete') {
          ...
          await axios.post(`/servers/${this.currentServer}/reset/${id}`, {
            use_nginx: this.confirmUseNginx,
          }, { headers: this.getAuthHeaders() })
        } else {
...
.btn-purge { background: #2f2f2f; color: #fff; }
.btn-purge:hover:not(:disabled) { background: #000; }
...
.btn-warn:hover { background: #ffa940; }
